...
import argparse, base64, json, math, os, random, ssl, subprocess, threading, time, typing, urllib.error, urllib.request
...
PAD = 0
TOPK = 20
SCAFFOLD_TEXT = (  # the empty thought block the chat template leaves to the model
    "<|channel>thought\n<channel|>"
)
SCAFFOLD: typing.Any = None
THOUGHT_OPEN: typing.Any = None
THOUGHT_CLOSE: typing.Any = None
...
def parse_schema(value):
  ...
  for q in value["questions"]:
    ...
    seen.add(qid)
    kind = q.get("type")
    orig_type = q.get("orig_type") or kind
    ...
    if not isinstance(ask_if, dict) or not all(
      ...
      raise SchemaError(
          f"question {qid!r}: ask_if must map a question id to a non-empty list"
          " of its answers"
      )
    qs.append({
        "id": qid,
  ...
  for q in qs:
    ...
    for dep, vals in q["ask_if"].items():
      ...
      if any(v not in names for v in vals):
        raise SchemaError(
            f"question {q['id']!r}: ask_if values for {dep!r} must be among"
            f" {names}"
        )
  ...
  if chunk_rows is not None and (
      not isinstance(chunk_rows, int) or chunk_rows < 8
  ):
  ...
  if (
    ...
    raise SchemaError(
        "schema: think must be a thought budget in tokens, 0 to 4096"
    )
  return {
      "questions": qs,
...
FORMATS = {
    "lines": (
        ...
        "{id}: ",
        (
            'Reply with one line per question, in this order, formatted as "id:'
            ' label".'
        ),
    ),
    "indexed": (
...
def system_text(schema, chunked=False):
  ...
  if chunked:
    s += (
        " A reply may cover only some of the questions; answer every line that"
        " is present."
    )
...
def answer_text(qs, labels, fmt="lines"):
  ...
  return join.join(
      lead.format(id=q["id"]) + q["labels"][l] for q, l in zip(qs, labels)
  )
...
def init_tokenizer(tok):
  ...
  assert (
      THOUGHT_OPEN + THOUGHT_CLOSE == SCAFFOLD
...
def resolve_template(qs, head, lead, fmt):
  ...
  Every label must change exactly one token, at the same position for all of a
  question's labels, or this raises SchemaError. ``head`` is the token run the
  canvas starts with: the empty thought block for a plain read, and empty when
  the prompt already ends the thought channel. ``lead`` is the text before the
  first answer: the join when earlier answers are in the prompt, so the tokens
  match one joint template.
  """
  ...
  for qi, q in enumerate(qs):
    ...
    for li in range(1, len(q["labels"])):
      ...
      if len(e) != len(base):
        raise SchemaError(
            f"question {q['id']!r}: label {q['labels'][li]!r} is not a single"
            " token"
        )
...
def template_for(schema, head, lead):
  ...
  if key not in _template_cache:
    _template_cache[key] = resolve_template(
        schema["questions"], head, lead, fmt
    )
...
def think_chat(sys_text, state_content, budget):
  ...
  if (
      ids[: len(THOUGHT_OPEN)] == THOUGHT_OPEN
  ):  # the model opens the channel itself
  ...
  for q in qs:
    ...
    rows = (
        ...
        + len(
            enc(
                answer_text(
                    trial, [0] * len(trial), schema.get("format", "lines")
                )
            )
        )
        + 1
...
def decide(schema, state_content, seed):
  """One decision. Questions run in stages by their dependencies.

  A stage is one joint read, chunked by the canvas, with a question marked alone
  in its own read. Later stages condition on every earlier answer: a prefilled
  continuation for a text state, or the answers restated in the state for an
  image. A question whose ask_if condition failed is skipped and its answer is
  null.
  """
  started = time.time()
  qs = [
  ...
  def run(group, k, conditioned):
    ...
    sub["think"] = (
        0 if (chained and text_state) or conditioned else schema["think"]
    )
    if text_state:
      if conditioned:
  ...
  def absorb(group, body, rows):
    ...
    lines.append(
        answer_text(
            group,
            [
                q["labels"].index(body["answers"][q["id"]]["label"])
                for q in group
            ],
            fmt,
        )
  ...
  for level in levels:
    ...
    if schema["sequential"] or len(groups) == 1:
      ...
      with ThreadPoolExecutor(max_workers=len(groups)) as ex:
        results = list(
            ex.map(
                lambda gk: run(gk[1], gk[0], conditioned),
  ...
  diagnostics = {
      ...
      "samples": (
          ...
          if one
              ...
              "policy": [
                  b["diagnostics"]["samples"]["policy"] for b, _ in parts
              ],
          }
      ),
      "timing": {
      ...
      "prompt_tokens": (
          max((b["diagnostics"].get("prompt_tokens") or 0) for b, _ in parts)
          or None
      ),
      "questions": diag_q,
      "engine": "vllm",
...
def decide_group(schema, sys_text, state_content, seed, prefix=None, lead=""):
  ...
  if prefix is None and schema["think"]:
    if isinstance(state_content, str):
      ...
      fits = (
          CANVAS_LEN - 1 - len(THOUGHT_OPEN) - len(THOUGHT_CLOSE) - len(answer)
      )
      if fits < 8:
        raise SchemaError(
            f"think: the canvas leaves {fits} rows for a thought beside this"
            " template"
        )
  ...
  for qi, q in enumerate(schema["questions"]):
    ...
    if n > 1:
      ...
      a["stderr"] = (var / n) ** 0.5
      a["agreement"] = (
          sum(1 for p in per if max(range(len(p)), key=lambda l: p[l]) == top)
          / n
      )
    answers[q["id"]] = a
    diag_q[q["id"]] = {
  ...
  tops = [
      {
          q["id"]: [
              q["labels"][
                  max(
                      range(len(r[qi]["probs"])),
                      key=lambda l: r[qi]["probs"][l],
                  )
              ],
              max(r[qi]["probs"]),
              r[qi]["entropy"],
...
def jev_schema(body):
  ...
  for qid, q in qs.items():
    ...
    if kind in ("noul", "boolean", "bool"):
      item["type"] = "noul"
      if crit is not None and not isinstance(crit, dict):
        raise SchemaError(
            f"question {qid!r}: noul criteria must be an object with true and"
            " false"
        )
      item["criteria"] = crit
    elif kind == "choice":
      ...
      if not isinstance(crit, list):
        raise SchemaError(
            f"question {qid!r}: score criteria must be an ordered list of"
            " levels"
        )
      item["levels"] = crit
    else:
...
def jev_images(value):
  ...
  for i, im in enumerate(value or []):
    if isinstance(im, str) and im.startswith("data:image/"):
      ...
      raise SchemaError(
          f"images[{i}]: a data:image/... URL or an object with content_type"
          " and base64"
      )
  return parts
...
def jev_answer(q, a):
  if a is None:
    return None
  if q["type"] == "noul":
    if q.get("orig_type") == "boolean":
      return {"type": "boolean", "noul": a["noul"], "probability": a["noul"]}
...
class Handler(BaseHTTPRequestHandler):
  ...
  def do_GET(self):
    if self.path == "/v1/models":
      model_id = getattr(ARGS, "model", "dgemma") if ARGS else "dgemma"
      return self._json(
          200,
          {
              "object": "list",
              "data": [
                  {"id": model_id, "object": "model", "owned_by": "djev-run"},
                  {
                      "id": "jev-latest",
                      "object": "model",
                      "owned_by": "djev-run",
    ...
    if self.path == "/health":
      ...
      if upstream:
        try:
          with urllib.request.urlopen(f"{upstream}/health", timeout=2) as r:
            if r.status != 200:
              return self._json(
                  503, {"status": "unavailable", "upstream": r.status}
              )
        except urllib.error.HTTPError as e:
          if e.code != 501:
            return self._json(503, {"status": "unavailable", "error": str(e)})
  ...
  def _read_request(self):
    ...
    for part in msg.iter_parts():
      ...
      if name == "request":
        assert isinstance(data, (bytes, bytearray, str))
  ...
  def do_POST(self):
    ...
    if self.path in ("/v1/systemone", "/v1/evaluate"):
  ...
  def _systemone(self, req, images):
    ...
    conf_map = {
        q["id"]: (
            body["answers"][q["id"]]["confidence"]
            if body["answers"].get(q["id"])
            else None
        )
        for q in schema["questions"]
    }
    labels = " ".join(
        f"{k}={v['label'] if v else 'skipped'}"
        for k, v in body["answers"].items()
    )
    print(
        f"systemone: {labels} reads={body['diagnostics']['timing']['reads']}"
    ...
    self._json(
        200,
        {
            ...
            "usage": {
                "input_tokens": (
                    body["diagnostics"].get("prompt_tokens")
                    or (
  ...
  def _chat(self, req):
    ...
    try:
      ...
      return self._json(
          400,
          {
              "error": {
                  "message": (
                      "system must be a JSON question schema and user must be"
                      f" JSON state or image parts: {e}"
                  ),
                  "type": "invalid_request_error",
              }
          },
    ...
    print(
        f"structured: {labels} reads={body['diagnostics']['timing']['reads']}"
        f" {body['diagnostics']['timing']['total_ms']:.0f}ms",
        flush=True,
    )
    self._json(
...
def self_signed(cert_dir):
  ...
  cert, key = os.path.join(cert_dir, "djev.crt"), os.path.join(
      cert_dir, "djev.key"
  )
...
def main():
  ...
  p.add_argument(
      "--tokenizer", default="/models/dgemma", help="HF id or local path"
  )
  p.add_argument(
      "--canvas", type=int, default=64, help="the served canvas length"
  )
  p.add_argument(
      "--canvas-step",
      type=int,
  ...
  p.add_argument(
      ...
      type=int,
      default=0,
      help="also listen with HTTPS here (0 = off)",
  )
  p.add_argument(
      "--cert-dir",
  ...
  tok = __import__("transformers").AutoTokenizer.from_pretrained(ARGS.tokenizer)
  if tok.chat_template is None and os.path.exists(
      os.path.join(ARGS.tokenizer, "chat_template.jinja")
  ):
  ...
  print(
      f"structured server on {ARGS.host}:{ARGS.port} -> {ARGS.upstream} (canvas"
      f" {CANVAS_LEN})",
      flush=True,
  )
