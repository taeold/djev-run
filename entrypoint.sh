...
"$PY_BIN" -m compileall -f -q \
  "$SITE_PKG/vllm/model_executor/models/diffusion_gemma.py" \
  "$SITE_PKG/vllm/utils/diffusion.py" \
  "$SITE_PKG/vllm/v1/core/sched/diffusion_scheduler.py" \
...
# 4. In-process vLLM LLM engine + structured_server.py (/v1/systemone, /v1/chat/completions, /snake, /dino, /tetris)
...
p = pathlib.Path(vllm_spec.origin).resolve().parent / "v1/worker/gpu_worker.py"
s = p.read_text()
...
print(f"[djev-init] HTTP listener active on {S.ARGS.host}:{S.ARGS.port} at t={time.time() - T_BOOT:.2f}s", flush=True)
...
def _bg_load_tokenizer():
    ...
    print(f"[djev-init] Tokenizer initialized at t={time.time() - T_BOOT:.2f}s", flush=True)
